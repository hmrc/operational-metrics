...
trait SchedulerUtils extends Logging:
  ...
  , ec                  : ExecutionContext
    ...
    if schedulerConfig.enabled then
      ...
      logger.info(s"Scheduler $label enabled, running every $frequency (after initial delay $initialDelay)")
      val cancellable =
  ...
  def scheduleWithLock(
        label          : String
      , schedulerConfig: SchedulerConfig
      , lockService    : ScheduledLockService
      )(f: => Future[Unit]
      )( using
        actorSystem         : ActorSystem
      , applicationLifecycle: ApplicationLifecycle
    ...
    schedule(label, schedulerConfig) {
      lockService.withLock(f).map:
        case Some(_) => logger.debug(s"Scheduler $label finished - releasing lock")
        case None    => logger.debug(s"Scheduler $label cannot run - lock ${lockService.lockId} is taken... skipping update")
    }
